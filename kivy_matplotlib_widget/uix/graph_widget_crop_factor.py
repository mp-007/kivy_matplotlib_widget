...
class MatplotFigureCropFactor(Widget):
    ...
    def transform_with_touch(self, event):
        ...
        event_ppos = (
            event.ppos[0] / self.crop_factor,
            event.ppos[1] / self.crop_factor,
        )
        event_pos = (
            event.pos[0] / self.crop_factor,
            ...
            if scale != "linear":
                try:
                    x_left, x_right = self.inv_transform_eval(
                        x_left, ax.xaxis
                    ), self.inv_transform_eval(x_right, ax.xaxis)
                except OverflowError:  # Limit case
                    x_left, x_right = min_, max_
                    if x_left <= 0.0 or x_right <= 0.0:  # Limit case
                        x_left, x_right = min_, max_
            ...
            if yscale != "linear":
                try:
                    y_left, y_right = self.inv_transform_eval(
                        y_left, ax.yaxis
                    ), self.inv_transform_eval(y_right, ax.yaxis)
                except OverflowError:  # Limit case
                    y_left, y_right = ymin_, ymax_
                    if y_left <= 0.0 or y_right <= 0.0:  # Limit case
                        y_left, y_right = ymin_, ymax_
    ...
    def apply_pan(self, ax, event, mode="pan"):
        ...
        xpress, ypress = trans.transform_point(
            (
                (
                    self._last_touch_pos[event][0]
                    - self.pos[0] / self.crop_factor
                ),
                (
                    self._last_touch_pos[event][1]
                    - self.pos[1] / self.crop_factor
                ),
            )
        )
        ...
        if not mode == "pan_y" and not mode == "adjust_y":
            if mode == "adjust_x":
                ...
                if self.anchor_x == "left":
                    if xdata > cur_xlim[0]:
                        ...
                        if self.stop_xlimits is not None:
                            if (
                                cur_xlim[0] > self.stop_xlimits[0]
                                and cur_xlim[1] < self.stop_xlimits[1]
                            ):
                                if inverted_x:
                ...
                if self.stop_xlimits is not None:
                    if (
                        cur_xlim[0] < self.stop_xlimits[0]
                        or cur_xlim[1] > self.stop_xlimits[1]
                    ):
        ...
        if self.do_zoom_x:
            ...
            if scale != "linear":
                try:
                    x_left, x_right = self.inv_transform_eval(
                        x_left, ax.yaxis
                    ), self.inv_transform_eval(x_right, ax.yaxis)
                except OverflowError:  # Limit case
                    x_left, x_right = min_, max_
                    if x_left <= 0.0 or x_right <= 0.0:  # Limit case
                        x_left, x_right = min_, max_
        ...
        if self.collide_point(*pos) and self.show_cursor:
            ...
            if self.hover_instance:
                ...
                self.hover_instance.xmin_line = (
                    float(ax.bbox.bounds[0]) * self.crop_factor + self.x
                )
                ...
                self.hover_instance.ymin_line = (
                    float(ax.bbox.bounds[1]) * self.crop_factor + self.y
                )
                ...
                if (
                    self.hover_instance.x_hover_pos
                    > self.x
                    + (self.axes.bbox.bounds[2] + self.axes.bbox.bounds[0])
                    * self.crop_factor
                    or self.hover_instance.x_hover_pos
                    < self.x + self.axes.bbox.bounds[0] * self.crop_factor
                    or self.hover_instance.y_hover_pos
                    > self.y
                    + (self.axes.bbox.bounds[1] + self.axes.bbox.bounds[3])
                    * self.crop_factor
                    or self.hover_instance.y_hover_pos
                    < self.y + self.axes.bbox.bounds[1] * self.crop_factor
                ):
    ...
    def update_hover(self):
        ...
        if self.hover_instance:
            ...
            if (
                ...
                and self.x_hover_data
                and self.y_hover_data
            ):
                ...
                self.hover_instance.xmin_line = (
                    float(self.axes.bbox.bounds[0]) * self.crop_factor + self.x
                )
                self.hover_instance.xmax_line = (
                ...
                self.hover_instance.ymin_line = (
                    float(self.axes.bbox.bounds[1]) * self.crop_factor + self.y
                )
                self.hover_instance.ymax_line = (
