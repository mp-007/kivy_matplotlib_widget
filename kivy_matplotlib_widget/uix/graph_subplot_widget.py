...
class MatplotFigureSubplot(MatplotFigure):
    ...
    def autoscale(self):
        ...
        for i, ax in enumerate(axes):
            ...
            lim_collection, invert_xaxis, invert_yaxis = self.data_limit_collection(
                ax, visible_only=self.autoscale_visible_only)
            if lim_collection:
                xchanged = False
        ...
        if self.hover_on:
            ...
            if self.compare_xdata:
                ...
                if not hasattr(line, 'axes'):
                    if hasattr(line, '_ContainerArtist__keep_alive'):
                        if self.hist_range and isinstance(line, BarContainer):
                            x_hist, y_hist, width_hist, height_hist = line[sel.index].get_bbox(
                            ).bounds
                            if self.cursor_xaxis_formatter:
                                custom_x = f"{
                ...
                if hasattr(self.myevent.inaxes, 'patch'):
                    if sel.artist is not self.myevent.inaxes.patch and hasattr(
                            sel.artist, 'get_cursor_data'):
                ...
                if self.hover_instance:
                    ...
                    if extra_data is not None:
                        self.hover_instance.label_y_value += ' [' + str(
                            extra_data) + ']'
                    ...
                    if self.highlight_hover:
                        ...
                        if self.last_line is None:
                            ...
                            ax.figure.canvas.draw_idle()
                            ax.figure.canvas.flush_events()
                            self.background_highlight = ax.figure.canvas.copy_from_bbox(
                                ax.figure.bbox)
                            self.last_line = line
        ...
        for i, ax in enumerate(self._pick_info):
            ...
            if self.interactive_axis and self.touch_mode == 'pan' and not self.first_touch_pan == 'pan':
                ...
                # cur_xlim[1] and inverted_x):
                    ...
                    bottom_anchor_zone = (
                        top_lim - bottom_lim) * .2 + bottom_lim
                ...
                if mode == 'adjust_x':
                    if self.current_anchor_axis == ax:
                        if self.anchor_x is None:
                            midpoint = (
                                (self.x + ax.bbox.bounds[2] + ax.bbox.bounds
                                 [0]) + (self.x + ax.bbox.bounds[0])) / 2
                            if event.x > midpoint:
                                if inverted_x:
                                    self.anchor_x = 'right'
                                else:
                ...
                if mode == 'adjust_y':
                    if self.current_anchor_axis == ax:
                        if self.anchor_y is None:
                            midpoint = (
                                (self.y + ax.bbox.bounds[3] + ax.bbox.bounds
                                 [1]) + (self.y + ax.bbox.bounds[1])) / 2
                            if event.y > midpoint:
                                if inverted_y:
                    ...
                    if self.text_instance:
                        if not self.text_instance.show_text:
                            ...
                            if event.y > midpoint:
                                ...
                                self.text_instance.y_text_pos = float(
                                    self.y + ax.bbox.bounds[1] + ax.bbox.bounds
                                    [3]) - self.text_instance.text_height
                                self.text_instance.offset_text = False
                            else:
                                anchor = 'bottom'
                                self.text_instance.x_text_pos = float(
                    ...
                    if self.text_instance:
                        if not self.text_instance.show_text:
                            ...
                            if event.y > midpoint:
                                anchor = 'top'
                                self.text_instance.x_text_pos = float(
                                    self.x + ax.bbox.bounds[0] + ax.bbox.bounds[2]) + dp(40)
                                self.text_instance.y_text_pos = float(
                                    self.y + ax.bbox.bounds[1] + ax.bbox.bounds
                                    [3]) - self.text_instance.text_height
                                self.text_instance.offset_text = True
                        ...
                        if not self.text_instance.show_text:
                            ...
                            if event.x < midpoint:
                                ...
                                self.text_instance.x_text_pos = float(
                                    self.x + ax.bbox.bounds[0])
                                self.text_instance.y_text_pos = float(
                                    self.y + ax.bbox.bounds[1] + ax.bbox.bounds[3]) + dp(2)
                                self.text_instance.offset_text = False
                            else:
                                anchor = 'right'
                                self.text_instance.x_text_pos = float(
    ...
    def update_lim(self):
        ...
        for index, ax in enumerate(self.box_axes):
            ...
            if xright > xleft:
                ...
                ax.set_xlim(
                    right=min(
                        self.x0_box[index], self.x1_box[index]), left=max(
                        self.x0_box[index], self.x1_box[index]))
            if ytop > ybottom:
                ax.set_ylim(
                    bottom=min(
                        self.y0_box[index], self.y1_box[index]), top=max(
                        self.y0_box[index], self.y1_box[index]))
            else:
                ax.set_ylim(
                    top=min(
                        self.y0_box[index],
