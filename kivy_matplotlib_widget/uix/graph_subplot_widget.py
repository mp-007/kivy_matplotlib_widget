...
class MatplotFigureSubplot(MatplotFigure):
    ...
    def my_in_axes(self, ax, mouseevent):
        ...
        if not result1:
            ...
            if (
                ylabelleft
                and mouseevent.x > ax.bbox.bounds[0] - ylabelsize
                and mouseevent.x < ax.bbox.bounds[0]
                and mouseevent.y < ax.bbox.bounds[1] + ax.bbox.bounds[3]
                and mouseevent.y > ax.bbox.bounds[1]
            ):
                result2 = True
            ...
            # x bottom axis
                ...
                xlabelbottom
                and mouseevent.x > ax.bbox.bounds[0]
                and mouseevent.x < ax.bbox.bounds[2] + ax.bbox.bounds[0]
                and mouseevent.y > ax.bbox.bounds[1] - xlabelsize
                and mouseevent.y < ax.bbox.bounds[1]
            ):
                result2 = True
            ...
            # #x top axis
                ...
                and mouseevent.y > ax.bbox.bounds[1] + ax.bbox.bounds[3]
                and mouseevent.y
                < ax.bbox.bounds[1] + ax.bbox.bounds[3] + xlabelsize
            ):
                result2 = True
    ...
    def on_figure(self, obj, value):
        ...
        if len(self.figure.axes) > 0 and self.figure.axes[0]:
            ...
            for ax in self.figure.axes:
                ...
                if hasattr(ax, "PolarTransform"):
                    ...
                    # note: make an other widget if you need polar graph with
                    # other type of graph
        ...
        if self.auto_cursor and len(self.figure.axes) > 0:
            ...
            # create maplotlib text and cursor (if needed)
            self.register_lines(lines)
    ...
    def autoscale(self):
        ...
        for i, ax in enumerate(axes):
            twinx = any(
                ax.get_shared_x_axes().joined(ax, prev) for prev in axes[:i]
            )

            twiny = any(
                ax.get_shared_y_axes().joined(ax, prev) for prev in axes[:i]
            )
            ...
            if twiny:
                autoscale_axis = "x"
            no_visible = self.myrelim(
                ax, visible_only=self.autoscale_visible_only
            )
            ax.autoscale_view(
            ...
            lim_collection, invert_xaxis, invert_yaxis = (
                self.data_limit_collection(
                    ax, visible_only=self.autoscale_visible_only
                )
            )
            if lim_collection:
    ...
    def get_data_xy(self, x, y):
        ...
        self.myevent_cursor.inaxes = self.figure.canvas.inaxes(
        ...
        if axes:
            ...
            if self.cursor_xaxis_formatter:
                ...
                x_format = (
                    self.axes.xaxis.get_major_formatter().format_data_short(
                        xdata
                    )
                )
            ...
            if self.cursor_yaxis_formatter:
                ...
                y_format = (
                    self.axes.yaxis.get_major_formatter().format_data_short(
                        ydata
                    )
                )
        ...
        if self.hover_on:
                    ...
                    if self.highlight_hover:
                        ...
                        if not axes:
                            ...
                            if self.last_line:
                                ...
                                ax = self.axes
                                if self.background:
                                    ax.figure.canvas.restore_region(
                                        self.background
                                    )
                                    # draw (blit method)
            ...
            if self.compare_xdata:
                    ...
                    if hasattr(line, "_ContainerArtist__keep_alive"):
                        if self.hist_range and isinstance(line, BarContainer):
                            x_hist, y_hist, width_hist, height_hist = (
                                line[sel.index].get_bbox().bounds
                            )
                            if self.cursor_xaxis_formatter:
                                custom_x = f"{
                                    self.cursor_xaxis_formatter.format_data(x_hist)}-{
                                    self.cursor_xaxis_formatter.format_data(
                                        x_hist + width_hist)}"
                            else:
                        ...
                        if line_label in self.hover_instance.children_names:
                            ...
                            index = [
                                ...
                                for ii, x in enumerate(
                                    self.hover_instance.children_names
                                )
                                if x == line_label and ii in index_list
                            ][0]
                            ...
                            if (
                                ...
                                available_widget[index].custom_color = (
                                    get_color_from_hex(
                ...
                self.hover_instance.xmax_line = (
                    float(ax.bbox.bounds[0] + ax.bbox.bounds[2]) + self.x
                )
                self.hover_instance.ymin_line = (
                    float(ax.bbox.bounds[1]) + self.y
                )
                self.hover_instance.ymax_line = (
                    float(ax.bbox.bounds[1] + ax.bbox.bounds[3]) + self.y
                )
                ...
                if not hasattr(line, "axes"):
                    if hasattr(line, "_ContainerArtist__keep_alive"):
                        ...
                        if self.hist_range and isinstance(line, BarContainer):
                            ...
                            if (
                                line._ContainerArtist__keep_alive[
                                    0
                                ...
                                invert_xy = True
                                x = line._ContainerArtist__keep_alive[
                                    0
                                ].container.datavalues[sel.index]
                            if self.cursor_xaxis_formatter:
                                custom_x = f"{
                                    self.cursor_xaxis_formatter.format_data(x_hist)}-{
                                    self.cursor_xaxis_formatter.format_data(
                ...
                if hasattr(self.myevent.inaxes, "patch"):
                    if (
                        sel.artist is not self.myevent.inaxes.patch
                        and hasattr(sel.artist, "get_cursor_data")
                    ):
                ...
                if self.hover_instance:
                    ...
                    if extra_data is not None:
                        self.hover_instance.label_y_value += (
                            " [" + str(extra_data) + "]"
                        )
                    ...
                    if (
                        ...
                        > self.y + ax.bbox.bounds[1] + ax.bbox.bounds[3]
                        or self.hover_instance.y_hover_pos
                    ...
                    if self.highlight_hover:
                        ...
                        if self.last_line is None:
                            ...
                            self.background_highlight = (
                                ax.figure.canvas.copy_from_bbox(ax.figure.bbox)
                            )
                            self.last_line = line
                            ...
                            self.hover_instance.custom_color = (
                                get_color_from_hex(to_hex(line.get_color()))
                            )
                            self.last_line_prop = {}
                            for key in self.highlight_prop:
                                line_attr = getattr(line, "get_" + key)
                ...
                if self.background is None:
                    ...
                    self.background = self.figcanvas.copy_from_bbox(
                        ax.figure.bbox
                    )
                    self.set_cross_hair_visible(True)
                    if self.last_line is not None:
    ...
    def apply_pan(self, my_ax, event, mode="pan"):
        ...
        if not self._pick_info:
            self.myevent.x = event.x - self.pos[0]
            self.myevent.y = event.y - self.pos[1]
            self.myevent.inaxes = self.figure.canvas.inaxes((event.x, event.y))
            # press event
            axes = [
                a
                for a in self.figure.canvas.figure.get_axes()
        ...
        for i, ax in enumerate(self._pick_info):
            ...
            if (
                ...
                # if (ydata < cur_ylim[0] and not inverted_y) or (ydata >
                # cur_ylim[1] and inverted_y):
                if (
                    xlabelbottom
                    and event.x > self.x + ax.bbox.bounds[0]
                    and event.x
                    < self.x + ax.bbox.bounds[2] + ax.bbox.bounds[0]
                    and event.y > self.y + ax.bbox.bounds[1] - xlabelsize
                    ...
                    if (
                        event.x < left_anchor_zone
                        or event.x > right_anchor_zone
                    ):
                        mode = "adjust_x"
                ...
                # elif (xdata < cur_xlim[0] and not inverted_x) or (xdata >
                # cur_xlim[1] and inverted_x):
                elif (
                    ylabelleft
                    and event.x > self.x + ax.bbox.bounds[0] - ylabelsize
                    and event.x < self.x + ax.bbox.bounds[0]
                    and event.y
                    < self.y + ax.bbox.bounds[1] + ax.bbox.bounds[3]
                    and event.y > self.y + ax.bbox.bounds[1]
                    ...
                    if (
                        event.y < bottom_anchor_zone
                        or event.y > top_anchor_zone
                    ):
                        mode = "adjust_y"
                        self.current_anchor_axis = ax
                    else:
    ...
    def update_hover(self):
        ...
        if self.hover_instance:
            ...
            if (
                ...
                xy_pos = self.cursor_last_axis.transData.transform(
    ...
    def min_max(self, event):
        ...
        for ax in axes:
            ...
            if (
                ...
                ylabelright
                and event.x
                < self.x + ax.bbox.bounds[2] + ax.bbox.bounds[0] + ylabelsize
                and event.x > self.x + ax.bbox.bounds[2] + ax.bbox.bounds[0]
                and event.y < self.y + ax.bbox.bounds[1] + ax.bbox.bounds[3]
                and event.y > self.y + ax.bbox.bounds[1]
            ):
                ...
                and event.y > self.y + ax.bbox.bounds[1] + ax.bbox.bounds[3]
                and event.y
                < self.y + ax.bbox.bounds[1] + ax.bbox.bounds[3] + xlabelsize
            ):
                right_lim = self.x + ax.bbox.bounds[2] + ax.bbox.bounds[0]
                left_lim = self.x + ax.bbox.bounds[0]
                left_anchor_zone = (right_lim - left_lim) * 0.2 + left_lim
    ...
    def update_lim(self):
        ...
        for index, ax in enumerate(self.box_axes):
            ...
            if xright > xleft:
                ax.set_xlim(
                    left=min(self.x0_box[index], self.x1_box[index]),
                    right=max(self.x0_box[index], self.x1_box[index]),
                )
            else:
                ax.set_xlim(
                    right=min(self.x0_box[index], self.x1_box[index]),
                    left=max(self.x0_box[index], self.x1_box[index]),
                )
            if ytop > ybottom:
                ax.set_ylim(
                    bottom=min(self.y0_box[index], self.y1_box[index]),
                    top=max(self.y0_box[index], self.y1_box[index]),
                )
            else:
                ax.set_ylim(
                    top=min(self.y0_box[index], self.y1_box[index]),
                    bottom=max(self.y0_box[index], self.y1_box[index]),
                )
    ...
    def reset_box(self):
        ...
        if min(abs(self._box_size[0]), abs(self._box_size[1])) > self.minzoom:
            ...
            for ax in self.box_axes:
                trans = ax.transData.inverted()
                x0_box, y0_box = trans.transform_point(
                    (
                        self._box_pos[0] - self.pos[0],
                        self._box_pos[1] - self.pos[1],
                    )
                )
                x1_box, y1_box = trans.transform_point(
                    (
