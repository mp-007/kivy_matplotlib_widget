        ...
        if touch.grab_current is self:
            ...
            if self.selected_side == "new select" or self.selected_side == "modify":
                ...
                if self.figure_wgt.collide_point(*self.to_window(*touch.pos)):
                    ...
                    #trick to improve app fps  -> hide for he moment
                    # if self.selected_side != "pan" and self.selected_side != "modify":
                    #     if self.max_rate is not None:
                    #         if self.last_touch_time is None:
                    #             self.last_touch_time = time.time()
                                
                    #         elif time.time() - self.last_touch_time < self.max_rate:
                    #             return
                    #         else:
                    #             self.last_touch_time=None
...
class PainterWidget2(PaintCanvasBehavior, FloatLayout):
    ...
    def onselect(self, verts):
        if verts and self.shapes:           
            ...
            # self.figure_wgt.figure.axes[0].add_artist(path)
            if self.collection:
                self.ind = np.nonzero(path.contains_points(self.xys))[0] #xys collection.get_offsets()
                self.fc[:, -1] = self.alpha_other
                self.fc[self.ind, -1] = 1
...
class SpanSelect(FloatLayout):
    ...
    def find_closest_index_after_limit(self,arr, limit):

        # Filter the array for values greater than the limit
        greater_than_limit = np.where(arr > limit)[0]
        
        if greater_than_limit.size == 0:
            return -1  # Return -1 if no values are greater than the limit
        
        # Find the index of the smallest value greater than the limit
        closest_index = greater_than_limit[0]
        
        return closest_index
    
    def find_closest_index_before_limit(self,arr, limit):
        # Filter the array for values less than or equal to the limit
        less_than_limit = np.where(arr <= limit)[0]
        
        if less_than_limit.size == 0:
            return -1  # Return -1 if no values are less than or equal to the limit
        
        # Find the index of the largest value less than or equal to the limit
        closest_index = less_than_limit[-1]
